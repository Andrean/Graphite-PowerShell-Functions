...
#>
    ...
    $Config = Import-XMLConfig -ConfigPath $configFilePath

    # Get Last Run Time
    $sleep = 0

    $configFileLastWrite = (Get-Item -Path $configFilePath).LastWriteTime
    ...
    while ($true)
    {
        ...
        if(-not $ExcludePerfCounters)
        {
            ...
            {
		        foreach($counterRename in $Config.CountersRename.GetEnumerator())
		        {
			        $sample.Path = $sample.Path -replace $counterRename.Name.toLower(), $counterRename.Value		
		        }
        ...
        if((Get-Item $configFilePath).LastWriteTime -gt (Get-Date -Date $configFileLastWrite)) {
            $Config = Import-XMLConfig -ConfigPath $configFilePath
        }
